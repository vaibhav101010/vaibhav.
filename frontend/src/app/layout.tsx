import type { Metadata } from 'next';
import './globals.css';
import CustomCursor from '@/components/CustomCursor';
import SmoothScrolling from '@/components/SmoothScrolling';
import Preloader from '@/components/Preloader';
import Chatbot from '@/components/Chatbot';

export const metadata: Metadata = {
  title: 'M/S Suman Jewellers Mirzapur | Best Gold, Silver & Diamond Jewellery',
  description:
    'Looking for the best jewellery shop in Mirzapur? Visit M/S Suman Jewellers at Wellesley Ganj for luxury 18K/22K Gold, 925 Silver, and GIA Certified Diamonds.',
  keywords: [
    'Suman Jewellers Mirzapur',
    'M/S Suman Jewellers',
    'best jewellery shop in Mirzapur',
    'gold shop in Mirzapur',
    'diamond jewellery Mirzapur',
    'silver jewellery Mirzapur',
    'Wellesley Ganj jewellers',
    'IGI certified diamonds Mirzapur',
    'Trusted jewellery showroom in Mirzapur',
    'Top jewellers near me Mirzapur',
    'Best gold rate in Mirzapur',
    'Authentic diamond jewellery Mirzapur UP',
    'Silver gift items shop in Mirzapur',
    'Jewellery showroom near Vindhyachal',
    '22k hallmark gold jewellery',
    'gold necklace designs',
    'gold bangles for women',
    'lightweight gold earrings',
    'gold chains for men',
    'Certified diamond engagement rings',
    'diamond necklaces',
    'solitaire rings',
    'real diamond nose pins',
    'Pure silver anklets (payal)',
    'silver pooja thali set',
    'silver coins for gifting',
    'heavy silver bracelets',
    'Bridal gold jewellery set Mirzapur',
    'Wedding jewellery collection',
    'Gold mangalsutra latest designs',
    'Engagement rings for couples',
    'Anniversary diamond gifts',
    'Dhanteras gold offers',
    '100% BIS hallmark gold Mirzapur',
    'Custom jewellery design in Mirzapur',
    'Where to buy authentic gold in Mirzapur',
    'Exchange old gold for new Mirzapur'
  ],
  icons: {
    icon: '/images/logo-gold.png',
    apple: '/images/logo-gold.png',
  },
  openGraph: {
    title: 'M/S Suman Jewellers Mirzapur',
    description: 'Best Gold, Silver & Diamond Jewellery Shop in Mirzapur',
    type: 'website',
  },
  verification: {
    google: 'Kwq6jYyvdboP2VKue2jcl5EEpV-V_CZp6oaKZuFrN38',
  },
};

export default function RootLayout({ children }: { children: React.ReactNode }) {
  const jsonLd = {
    '@context': 'https://schema.org',
    '@type': 'JewelryStore',
    name: 'M/S Suman Jewellers',
    description: 'Luxury Diamond, Gold & Silver Jewellery in Mirzapur. GIA & IGI certified.',
    address: {
      '@type': 'PostalAddress',
      streetAddress: 'Wellesley Ganj, near Sai Baba Temple',
      addressLocality: 'Mirzapur',
      addressRegion: 'UP',
      addressCountry: 'IN'
    },
    url: 'https://www.sumanjeweller.com'
  };

  return (
    <html lang="en" suppressHydrationWarning>
      <head>
        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="anonymous" />
        <link
          href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,600;0,700;1,300;1,400&family=Inter:wght@300;400;500;600&display=swap"
          rel="stylesheet"
        />
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
        />
      </head>
      <body suppressHydrationWarning>
        <SmoothScrolling>
          <Preloader />
          <CustomCursor />
          {children}
          <Chatbot />
        </SmoothScrolling>
      </body>
    </html>
  );
}
